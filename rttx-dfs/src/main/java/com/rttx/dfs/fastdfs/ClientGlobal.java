...
package com.rttx.dfs.fastdfs;


import com.rttx.commons.utils.StringUtils;
import com.rttx.dfs.common.IniFileReader;
import com.rttx.dfs.common.MyException;
...
public class ClientGlobal {
  ...
  public static void init(FastdfsProperties properties) throws MyException, IOException {

    g_connect_timeout = properties.getConnectTimeoutInSeconds();
    if (g_connect_timeout < 0) {
      g_connect_timeout = DEFAULT_CONNECT_TIMEOUT;
    }
    g_connect_timeout *= 1000; //millisecond

    g_network_timeout = properties.getNetworkTimeoutInSeconds();
    if (g_network_timeout < 0) {
      g_network_timeout = DEFAULT_NETWORK_TIMEOUT;
    }
    g_network_timeout *= 1000; //millisecond
    ...
    if (StringUtils.isEmpty(properties.getTrackerServers())){
      throw new MyException("trackerServers is empty.");
    }

    initByTrackers(properties.getTrackerServers());

    g_tracker_http_port = properties.getHttpTrackerHttpPort();
    g_anti_steal_token = properties.isHttpAntiStealToken();
    g_secret_key = properties.getHttpSecretKey();
  }
