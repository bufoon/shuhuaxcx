...
@Configuration
@ConditionalOnProperty(name = "rttx.storage.db.readDatasource.enable", havingValue = "true")
...
public class ReadDsDruidConfig {
    ...
    public ReadDsDruidConfig(AppInfo appInfo, ZkService zkService, DbProperties localDbProperties,
        ...
        if (this.dbProperties == null){
            ...
            logger.info("读取本地的项目配置**********************************");
        } else {
            logger.info("读取[ZK]的项目配置**********************************");
        }
    }
    ...
    @Primary
    public MybatisSqlSessionFactoryBean readSqlSessionFactoryBean() throws Exception {
